...
$query="SELECT musician_.ID, musician_.Name, musician_.State,musician_.Address, musician_.Pincode,musician_.Description,musician_.Price FROM musician_ WHERE musician_.ID=".$_GET["id"];
//$result=mysql_query($query);
$query1="SELECT musician_pic.Picture,musician_pic.pic_no FROM musician_ INNER JOIN musician_pic ON musician_.ID=musician_pic.ID AND musician_.ID=".$_GET["id"];
$query2="SELECT musician_phone.Number FROM musician_ INNER JOIN musician_phone ON musician_.ID=musician_phone.ID AND musician_.ID=".$_GET["id"];
$query3="SELECT musician_.ID,musician_.Name,musician_.State,musician_.Address,musician_.Pincode FROM musician_ WHERE musician_.ID IN (SELECT E_Bookedid FROM history WHERE C_ID IN (SELECT C_ID FROM history WHERE E_Bookedid=".$_GET["id"].") AND E_Bookedid!=".$_GET["id"].")";
$query4="SELECT musician_pic.Picture FROM musician_pic WHERE musician_pic.ID IN (SELECT E_Bookedid FROM history WHERE C_ID IN (SELECT C_ID FROM history WHERE E_Bookedid=".$_GET["id"].") AND E_Bookedid!=".$_GET["id"].") AND musician_pic.pic_no=1";
...
function startAjax(){
   ...
   $.ajax({
     type: "POST",
     url: url,
     data: {"bdate" :$('#booking').find('input[name="bday"]').val(), "bookID" :book_id, "id_" :<?php echo $_GET["uid"]; ?>,"type":'MU'},
...
Tel-  <?php 
               ...
               echo '<img src="data:image/jpeg;base64,'.base64_encode( $row1['Picture'] ).'" style="height:420px;width:644px;"/>'; ?>
                                ...
                                <div class="row margin-bottom-30">
                                   ...
                                   <?php echo '<a href="musician_desc.php?id='.$row3['ID'].'&uid=" style="text-decoration: none" >';?>
								   <div class="col-sm-3 person-details">
                                        <figure style="width:242px;">
                                         <?php
                                             $query4="SELECT musician_pic.Picture FROM musician_pic WHERE musician_pic.ID=".$row3["ID"];
