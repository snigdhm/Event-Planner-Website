...
$query="SELECT hall.ID, hall.Name, hall.State,hall.Address, hall.Pincode,hall.Description,hall.Price,hall.AC,hall.Capacity,hall.H_ID,hall_owner.FName,hall_owner.LName FROM hall INNER JOIN hall_owner ON hall.H_ID=hall_owner.ID WHERE hall.ID=".$_GET["id"];
//$result=mysql_query($query);
$query1="SELECT hall_pic.Picture,hall_pic.pic_no FROM hall INNER JOIN hall_pic ON hall.ID=hall_pic.ID AND hall.ID=".$_GET["id"];
$query2="SELECT hall_phone.Number FROM hall_owner INNER JOIN hall_phone ON hall_owner.ID=hall_phone.ID WHERE hall_owner.ID=(SELECT hall.H_ID FROM hall WHERE hall.ID=".$_GET["id"].")";
$query3="SELECT hall.ID,hall.Name,hall.State,hall.Address,hall.Pincode FROM hall WHERE hall.ID IN (SELECT E_Bookedid FROM history WHERE C_ID IN (SELECT C_ID FROM history WHERE E_Bookedid=".$_GET["id"].") AND E_Bookedid!=".$_GET["id"].")";
$query4="SELECT hall_pic.Picture FROM hall_pic WHERE hall_pic.ID IN (SELECT E_Bookedid FROM history WHERE C_ID IN (SELECT C_ID FROM history WHERE E_Bookedid=".$_GET["id"].") AND E_Bookedid!=".$_GET["id"].") AND hall_pic.pic_no=1";
...
function startAjax(){
   ...
   $.ajax({
     type: "POST",
     url: url,
     data: {"bdate" :$('#booking').find('input[name="bday"]').val(), "bookID" :book_id, "id_" :<?php echo $_GET["uid"]; ?>,"type":'HA'},
...
<br>
<h3>Address</h3><br>
<p>Tentative Price- Rs.<?php echo $row['Price']; ?></p>
<p><?php if($row['AC']) echo "With AC"; else echo "Non-AC"?></p>
<p>Capacity- <?php echo $row['Capacity']; ?></p>
<p> Hall Owner- <?php echo $row['FName']; ?> <?php echo $row['LName']; ?></p>
...
Tel-  <?php 
               ...
               echo '<img src="data:image/jpeg;base64,'.base64_encode( $row1['Picture'] ).'" style="height:420px;width:644px;"/>'; ?>
                                ...
                                <div class="row margin-bottom-30">
                                   ...
                                   <?php echo '<a href="hall_desc.php?id='.$row3['ID'].'&uid=" style="text-decoration: none" >';?>
								   <div class="col-sm-3 person-details">
                                        <figure style="width:242px;">
                                         <?php
                                             $query4="SELECT hall_pic.Picture FROM hall_pic WHERE hall_pic.ID='".$row3["ID"]."'";
