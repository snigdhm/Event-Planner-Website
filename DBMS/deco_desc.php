...
$query="SELECT decorator_.ID, decorator_.Name, decorator_.State,decorator_.Address,decorator_.Pincode,decorator_.Description FROM decorator_ WHERE decorator_.ID=".$_GET["id"];
//$result=mysql_query($query);
$query1="SELECT deco_pic.Picture,deco_pic.pic_no FROM decorator_ INNER JOIN deco_pic ON decorator_.ID=deco_pic.ID WHERE decorator_.ID=".$_GET["id"];
$query2="SELECT deco_phone.Number FROM decorator_ INNER JOIN deco_phone ON decorator_.ID=deco_phone.ID WHERE decorator_.ID=".$_GET["id"];
$query3="SELECT decorator_.ID,decorator_.Name,decorator_.State,decorator_.Address,decorator_.Pincode FROM decorator_ WHERE decorator_.ID IN (SELECT E_Bookedid FROM history WHERE C_ID IN (SELECT C_ID FROM history WHERE E_Bookedid=".$_GET["id"].") AND E_Bookedid!=".$_GET["id"].")";
$query4="SELECT deco_pic.Picture FROM deco_pic WHERE deco_pic.ID IN (SELECT E_Bookedid FROM history WHERE C_ID IN (SELECT C_ID FROM history WHERE E_Bookedid=".$_GET["id"].") AND E_Bookedid!=".$_GET["id"].") AND deco_pic.pic_no=1";
...
function startAjax(){
   ...
   $.ajax({
     type: "POST",
     url: url,
     data: {"bdate" :$('#booking').find('input[name="bday"]').val(), "bookID" :book_id, "id_" :<?php echo $_GET["uid"]; ?>,"type":'DE'},
...
Tel-  <?php 
               ...
               echo '<img src="data:image/jpeg;base64,'.base64_encode( $row1['Picture'] ).'" style="height:420px;width:644px;"/>'; ?>
                                ...
                                <div class="row margin-bottom-30">
                                   ...
                                   <?php echo '<a href="deco_desc.php?id='.$row3['ID'].'&uid=" style="text-decoration: none" >';?>
								   <div class="col-sm-3 person-details">
                                        <figure style="width:242px;">
                                         <?php
                                             $query4="SELECT deco_pic.Picture FROM deco_pic WHERE deco_pic.ID=".$row3["ID"];
